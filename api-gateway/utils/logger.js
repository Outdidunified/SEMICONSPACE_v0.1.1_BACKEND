...
const logFilename = path.join(logsDirectory, 'APIGatewayLog.log');
...
const logger = winston.createLogger({
  ...
  level: 'pingpong',  // Allow all log levels
  format: winston.format.timestamp({ format: () => moment().format('DD/MM/YYYY hh:mm:ss A') }),
  transports: [
    new winston.transports.Console({
      format: winston.format.combine(
        winston.format.timestamp({ format: () => moment().format('DD/MM/YYYY hh:mm:ss A') }),
        consoleFormat
      )
    }),
